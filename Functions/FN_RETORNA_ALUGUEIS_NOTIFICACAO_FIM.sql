CREATE OR REPLACE FUNCTION FN_RETORNA_ALUGUEIS_NOTIFICACAO_FIM(OP integer,P_USUARIO TEXT DEFAULT '')
...
BEGIN
    ...
    WHERE COD_STATUS_ALUGUEL = 2
      AND NOT notificacao_devolucao
      AND ((OP = 1 AND TO_CHAR(data_devolucao - INTERVAL '2' HOUR,'YYYY-MM-DD HH24') = TO_CHAR(CURRENT_TIMESTAMP,'YYYY-MM-DD HH24'))
        OR (OP = 2));

    UPDATE aluguel_encontro
    SET notificacao_devolucao = TRUE
    WHERE (ID_ALUGUEL IN (SELECT UUID(IDALUGUEL) FROM TEMP_ENTREGA));
